...
def show_subscription_gate():
    ...
    subscriber = get_subscription_metadata(current_key)

    st.sidebar.header("Subscription required")
    st.sidebar.write(
        "Enter your paid subscription key below to unlock Forex Tracker Pro."
    )
    ...
    if st.sidebar.button("Unlock app"):
        if is_subscription_key_valid(subscription_key):
            ...
            st.sidebar.error("Invalid subscription key.")

    if valid and subscriber:
        info_lines = []
        if subscriber.get("user"):
            info_lines.append(f"**User:** {subscriber['user']}")
        if subscriber.get("email"):
            info_lines.append(f"**Email:** {subscriber['email']}")
        if subscriber.get("plan"):
            info_lines.append(f"**Plan:** {subscriber['plan']}")
        if subscriber.get("expires"):
            info_lines.append(f"**Expires:** {subscriber['expires']}")
    ...
    if not valid:
        ...
        if SUBSCRIPTION_BACKEND_URL:
            if st.sidebar.button("Generate checkout link"):
                new_url = get_checkout_url_from_backend()
                if new_url:
                    st.session_state["checkout_url"] = new_url
                    st.rerun()
                else:
                    st.sidebar.error(
                        "Unable to generate checkout link. Check the backend and try again."
                    )

            if checkout_url:
                st.sidebar.markdown(
                    f"[👉 Subscribe now]({checkout_url})"
                )
                st.sidebar.write(
                    "Your Stripe checkout session link was generated successfully."
                )
            elif SUBSCRIPTION_LINK:
                st.sidebar.markdown(
                    f"[👉 Subscribe now]({SUBSCRIPTION_LINK})"
                )
                st.sidebar.write(
                    "Use your Stripe Checkout URL, Gumroad link, or other payment page."
                )
            else:
                st.sidebar.info(
                    "Set `SUBSCRIPTION_BACKEND_URL` to a checkout session endpoint or `SUBSCRIPTION_PAYMENT_LINK` to a direct payment URL."
                )
        elif SUBSCRIPTION_LINK:
            st.sidebar.markdown(
                f"[👉 Subscribe now]({SUBSCRIPTION_LINK})"
            ...
            st.sidebar.info(
                "Set `SUBSCRIPTION_PAYMENT_LINK` or `subscription_payment_link` in Streamlit secrets to add your payment link."
            )

        st.sidebar.markdown("---")
        st.sidebar.subheader("New here? Register for access")
        with st.sidebar.form("new_user_registration_form"):
            reg_name = st.text_input("Full name")
            reg_email = st.text_input("Email")
            reg_plan = st.selectbox("Desired plan", ["monthly", "annual", "lifetime", "custom"], index=0)
        ...
        if reg_submit:
            if not reg_name or not reg_email:
                ...
                pending_data = {
                    ...
                    "plan": reg_plan,
                    "notes": reg_notes,
                    "checkout_url": checkout_url or SUBSCRIPTION_LINK or ""
                }
                ...
                if email_ok:
                    st.sidebar.success("Registration request submitted. Admin has been notified.")
                else:
                    st.sidebar.warning(
                        "Registration request saved, but email notification is not configured or failed."
                    )

                if checkout_url:
                    st.sidebar.markdown(f"[👉 Subscribe now]({checkout_url})")
                elif SUBSCRIPTION_LINK:
                    st.sidebar.markdown(f"[👉 Subscribe now]({SUBSCRIPTION_LINK})")

        st.error(
            "Subscription required. Enter your key or subscribe via the sidebar link."
        )
        st.stop()
